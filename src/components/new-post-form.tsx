...
import type { PostTypeConfig } from "@/lib/posts";
...
export default function NewPostForm({ type, config }: { type: PostType; config: PostTypeConfig }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full rounded-lg border border-dashed border-slate-300 bg-white px-4 py-3 text-left text-sm text-slate-500 hover:border-emerald-500 hover:text-slate-700"
      >
        + {config.newLabel}
      </button>
    );
